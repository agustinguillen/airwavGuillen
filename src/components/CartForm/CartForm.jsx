...
const CartForm = ({ products, totalPrice }) => {
  ...
  const saveOrder = (e) => {
    ...
    let validFirstName =
      newOrder.name !== "" && new RegExp(/^.+$/g).test(newOrder.name);
    let validLastName =
      newOrder.name !== "" && new RegExp(/^.+$/g).test(newOrder.lastName);
    ...
    <>
      {error && (
        <Alert variant="danger" onClose={() => setError(false)} dismissible>
          Revisa que tus datos ingresados sean válidos!
        </Alert>
      )}
      <Form
        onSubmit={saveOrder}
      ...
      {showModal && (
        ...
        >
          ...
          <Modal.Footer>
            <Button variant="warning" onClick={() => finishPurchase()}>
              Listo!
            </Button>
          </Modal.Footer>
        </Modal.Dialog>
